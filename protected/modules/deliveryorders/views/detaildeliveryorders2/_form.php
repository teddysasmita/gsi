...
$itemScript=<<<EOS
      $('#Detaildeliveryorders2_itemname').focus(function(){
...
Yii::app()->clientScript->registerScript('itemscript', $itemScript, CClientScript::POS_READY);
 ...
 ?>
	...
	<div class="row">
		...
		<?php 
               echo CHtml::textField('Detaildeliveryorders2_itemname', lookup::ItemNameFromItemID($model->iditem) , array('size'=>50));   
                  ...
                  'options'=>array(
                      ...
                      'buttons'=>array(
                          array('text'=>'Ok', 'click'=>'js:function(){
                             $(\'#Detaildeliveryorders2_itemname\').val($(\'#dialog-item-name\').val());
                             $.get(\'index.php?r=LookUp/getItemID\',{ name: encodeURI($(\'#dialog-item-name\').val()) },
                                 function(data) {
                                    $(\'#Detaildeliveryorders2_iditem\').val(data);
                                 })
                             $(this).dialog("close");
                           }'),
